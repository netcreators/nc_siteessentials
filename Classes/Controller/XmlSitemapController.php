...
class XmlSitemapController extends XmlSitemapBaseController {
	...
	public function renderAction() {
		...
		if($this->settings['xml_sitemap']['makeChangeFreqRecursively'])
			$this->makeChangeFreqRecursively($GLOBALS['TSFE']->rootLine[0]['uid']);
		...
		return $this->renderSitemapContent();
	}
	 ...
	 *
	 * 	if($TSFE->originalShortcutPage) { // <-- No access.
	 * 		$id = $TSFE->originalShortcutPage['uid'];
	 * 	}
	 *
	 * @return bool
	 */
	protected function isCurrentPageShortcutTargetOfSiteRootPage() {
		...
		if(count($TSFE->rootLine) != 2) {
			return false;
		}

		// If site root page is not a shortcut, we can quit here.
		if($TSFE->rootLine[0]['doktype'] != \TYPO3\CMS\Frontend\Page\PageRepository::DOKTYPE_SHORTCUT) {
			return false;
		}
		...
		$siteRootPage = $TSFE->sys_page->getPage($TSFE->rootLine[0]['uid']);
		$shortcutPage = $TSFE->getPageShortcut(
			$siteRootPage['shortcut'],
			$siteRootPage['shortcut_mode'],
			$siteRootPage['uid']
		);

		return $shortcutPage['uid'] == $TSFE->id;
	}
}