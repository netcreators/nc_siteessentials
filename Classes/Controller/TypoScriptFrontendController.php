...
class TypoScriptFrontendController {
	...
	public function __construct() {
		...
		// saves content
		$this->content = $GLOBALS['TSFE']->content;
	...
	public function contentPostProcOutput() {
		...
		} else if($this->settings['content']['contentPostProcOutput']['metaGeneratorTag']['removeVersionNumber']) {
			
			$this->removeVersionNumberFromMetaGeneratorTag();
		}
			
		// returns the modified content
		$GLOBALS['TSFE']->content = $this->content;
	}
	...
	public function renderFrontendHyperlinkAttributes($config) {
		...
		foreach($document->getElementsByTagName('a') as $hyperlink) {
			...
			foreach($config as $currentConfig) {
				...
				// perform preg_match
				&& (trim($currentConfig['pregMatchHrefTag']) == '*' || preg_match($currentConfig['pregMatchHrefTag'], $hyperlink->getAttribute('href')))) {
					
					// adds a class
					if($currentConfig['addClass'])
						$hyperlink->setAttribute('class', ($hyperlink->hasAttribute('class') ? $hyperlink->getAttribute('class'). ' '.$currentConfig['addClass'] : $currentConfig['addClass']));
						
					// adds a attribute
					if($currentConfig['addAttribute'] && $currentConfig['addAttribute']['name'] && !$hyperlink->hasAttribute($currentConfig['addAttribute']['name']))
						$hyperlink->setAttribute($currentConfig['addAttribute']['name'], $currentConfig['addAttribute']['value']);
				}
			}	
		}
	...
	public function loadTypoScriptConfiguration($extKey) {
		...
		return $configurationManager->getConfiguration(\TYPO3\CMS\Extbase\Configuration\ConfigurationManagerInterface::CONFIGURATION_TYPE_SETTINGS, $extKey);
	}
